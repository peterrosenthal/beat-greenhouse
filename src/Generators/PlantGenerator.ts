  ...
  generate(): void {
    ...
    if (this.visualizeEnvelope) {
      ...
      for (let i = 0; i < layers; i++) {
        ...
        const first = i < mid ?
          lerp(this.envelopeNodes.firstBot, this.envelopeNodes.firstMid, i / mid):
          lerp(this.envelopeNodes.firstMid, this.envelopeNodes.firstTop, (i - mid) / mid);
        const second = i < mid ?
          lerp(this.envelopeNodes.secondBot, this.envelopeNodes.secondMid, i / mid):
          lerp(this.envelopeNodes.secondMid, this.envelopeNodes.secondTop, (i - mid) / mid);
        const third = i < mid ?
          lerp(this.envelopeNodes.thirdBot, this.envelopeNodes.thirdMid, i / mid):
          lerp(this.envelopeNodes.thirdMid, this.envelopeNodes.thirdTop, (i - mid) / mid);
        for (let j = 0; j < resolution; j++) {
          const t = j / resolution;
          let radius = t < 2 / 3 ? t < 1 / 3 ?
            lerp(first, second, t * 3):
            lerp(second, third, t * 3 - 1):
            lerp(third, first, t * 3 - 2);
    ...
    for (let i = 0; i < this.numAttractors; i++) {
      ...
      let radius = theta < 4 * Math.PI / 3 ? theta < 2 * Math.PI / 3 ?
        lerp(first, second, theta / (2 * Math.PI / 3)):
        lerp(second, third, theta / (2 * Math.PI / 3) - 1):
        lerp(third, first, theta / (2 * Math.PI / 3) - 2);
